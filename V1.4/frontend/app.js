...
function renderPendingCard(data, settings) {
  ...
  if (!settings.ai_enabled) {
    disabled = true;
    note = "AI integration is off. Turn it on in Settings to triage these findings.";
  } else if (!settings.anthropic_api_key_set) {
    disabled = true;
    note = "Add your Anthropic API key in Settings to triage these findings.";
  }
  ...
  card.innerHTML = `
    ...
    <button class="primary" type="button" id="approveTriageButton" ${disabled ? "disabled" : ""}>Approve AI triage</button>
    <p class="form-note" id="triageNote" style="margin-top: 8px;">${note}</p>
  `;
